...
async def verify_webhook(
    ...
    """
    # ── ENTRY LOG — first thing that runs ────────────────────────────────
    logger.info(
...
async def receive_webhook(
    ...
    # Always verify in production and staging. Skip in development for testing.
    ...
    for entry_idx, entry in enumerate(entries):
        ...
        for change_idx, change in enumerate(changes):
            ...
            logger.info(
                "WEBHOOK_POST_VALUE: phone_number_id=%s, message_count=%d, status_count=%d, contact_name=%s",
                phone_number_id, len(messages), len(statuses), contact_name,
            )

            for msg_idx, message in enumerate(messages):
                wa_message_id = message.get("id")
                if not wa_message_id:
                    logger.warning("WEBHOOK_POST_MSG_NO_ID: msg_idx=%d, message=%s", msg_idx, message)
                    continue
                ...
                if not msg_text:
                    logger.info(
                        "WEBHOOK_POST_SKIP_NONCTEXT: wa_message_id=%s, type=%s — "
                ...
                total_messages += 1
                logger.info(
                    "WEBHOOK_POST_PIPELINE_START: wa_message_id=%s, wa_id=%s, "
                    "phone_number_id=%s, type=%s",
                    wa_message_id, wa_id, phone_number_id, msg_type,
                )
