...
Model 1: per-business access token REMOVED.
  Platform uses ONE System User Access Token from env (WHATSAPP_DEFAULT_ACCESS_TOKEN).
  Each business only stores its whatsapp_phone_number_id for routing.
"""
...
class Business(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # NOTE: whatsapp_access_token_encrypted REMOVED in Model 1.
    #   Platform uses WHATSAPP_DEFAULT_ACCESS_TOKEN from env.
    #   DO NOT store access tokens per business.
    is_whatsapp_enabled: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False,
        comment="Toggle WhatsApp bot on/off per business without removing config"
    )
