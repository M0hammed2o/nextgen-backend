"""
Application settings — loaded from .env.backend.
Never hardcode secrets. All secrets come from env vars.

Model 1 Architecture:
  ONE Meta App → ONE WABA → ONE platform System User Access Token
  MULTIPLE phone_number_id values (one per client business)
  Token lives in env (WHATSAPP_DEFAULT_ACCESS_TOKEN), NOT per-business in DB.
"""
...
class Settings(BaseSettings):
    ...
    # ── Meta / WhatsApp (Model 1: Single WABA, one platform token) ───────
    ...
    WHATSAPP_DEFAULT_ACCESS_TOKEN: str = Field(
        default="CHANGE-ME",
        description=(
            "Platform System User Access Token (long-lived). "
            "Shared across ALL businesses under our WABA. "
